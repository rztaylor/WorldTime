...
import { locations, locationsForCountry } from '../../data/locations'
...
export function WorldMap() {
  ...
  const relatedCountries = useMemo(
    () => new Set(locations.filter((item) => item.timezone === active.timezone).map((item) => item.country)),
    [active.timezone],
  )

  const selectCountry = (name: string) => {
    const choices = locationsForCountry(name)
    if (!choices.length) return
    if (choices.length === 1) addLocation(choices[0])
    else selectLocation(choices[0])
  }

  return (
    <section id="map" className="map-panel" aria-label="Interactive world timezone map">
      <div className="timezone-bands" aria-hidden="true">
        {Array.from({ length: 25 }, (_, index) => <span key={index}>{index === 12 ? 'UTC' : index - 12 > 0 ? `+${index - 12}` : index - 12}</span>)}
      </div>
      <div className="map-canvas">
        ...
        <ComposableMap width={800} height={410} projectionConfig={{ scale: 126 }} role="img" aria-label="World map. Select a supported country to add its timezone.">
          <ZoomableGroup
          ...
          >
            <Geographies geography={world as never}>
              {({ geographies }) => geographies.map((geo) => {
                ...
                const supported = locationsForCountry(name).length > 0
                const selected = name === active.country
            ...
            <Marker coordinates={[active.longitude, active.latitude]}>
