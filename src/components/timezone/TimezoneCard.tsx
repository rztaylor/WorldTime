...
import { dayRelation, formatClock, offsetLabel, zoneName } from '../../lib/timezone'
  ...
  return (
    ...
    >
      <div className="card-topline">
        ...
        <span>{timezone.isHome ? <><Home /> Home</> : timezone.city}</span>
      ...
      {timezone.isHome && <strong className="home-city">{timezone.city}</strong>}
      <div className="card-time">{formatClock(now, timezone.timezone, timeFormat)}</div>
      <div className="card-meta"><strong>{zoneName(now, timezone.timezone)}</strong><span>{offsetLabel(now, timezone.timezone)}</span>{relation !== 'Today' && <em>{relation}</em>}</div>
